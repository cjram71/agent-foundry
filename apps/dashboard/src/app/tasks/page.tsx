...
export default async function TasksPage() {
  const tasks = await prisma.task.findMany({ orderBy: { createdAt: 'desc' }, include: { project: true, _count: { select: { agentRuns: true, approvals: true } } } });
  return <div className="page-stack"><header className="page-header"><div><p className="eyebrow">WORK QUEUE</p><h1>Tasks</h1><p>Plan, approve, execute, and review agent work from one place.</p></div><Link className="button primary" href="/tasks/new">+ New task</Link></header>
