...
export default async function ExecutiveHome() {
  ...
  return <div className="page-stack executive-home">
    ...
    <section className="company-journey panel">
      <div className="journey-head"><div><p className="eyebrow">YOUR NEXT STEP</p><h2>{setupStage === 'COMPANY_RECORD' ? 'Complete the company record' : setupStage === 'CONSTITUTION' ? 'Approve how AI may operate' : setupStage === 'DISCOVERY' ? 'Start company discovery' : 'Operate and improve Boosta'}</h2><p>{setupStage === 'COMPANY_RECORD' ? 'Boosta needs a verified company record before work can be governed.' : setupStage === 'CONSTITUTION' ? 'Review the company rules. Nothing autonomous starts when you approve them; approval only establishes the boundaries.' : setupStage === 'DISCOVERY' ? 'Run bounded public research across Boosta products, markets, finances, security and compliance. Independent Red Team findings are synthesized by the AI CEO for your decision.' : 'Review decisions, monitor approved work and measure business results.'}</p></div><span className="stage-label">{setupStage === 'OPERATING' ? 'Operating' : `Setup ${setupStage === 'COMPANY_RECORD' ? '1' : setupStage === 'CONSTITUTION' ? '2' : '3'} of 3`}</span></div>
      ...
      <div className="journey-action">{setupStage === 'COMPANY_RECORD' ? <Link className="button primary" href="/businesses">Open company settings</Link> : setupStage === 'CONSTITUTION' ? <><ConstitutionApproval/><Link className="button secondary" href="/businesses">Read all rules</Link></> : setupStage === 'DISCOVERY' ? <><StartDiscovery/><Link className="button secondary" href="/businesses">Review company information</Link></> : <Link className="button primary" href="/approvals">Review company decisions</Link>}</div>
    </section>
