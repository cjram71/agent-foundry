...
const worker = new Worker('foundry-tasks', async (job) => {
  ...
  try {
    ...
    await prisma.$transaction(async tx => {
      ...
      const missionLink = await tx.missionTask.findUnique({ where: { taskId: task.id }, select: { missionId: true } }); if (missionLink) { await tx.mission.update({ where: { id: missionLink.missionId }, data: { status: 'awaiting_human_review' } }); const existingMissionApproval = await tx.missionApproval.findFirst({ where: { missionId: missionLink.missionId, approvalType: 'plan', decision: 'pending' } }); if (!existingMissionApproval) await tx.missionApproval.create({ data: { missionId: missionLink.missionId, approvalType: 'plan' } }); await tx.missionEvent.create({ data: { missionId: missionLink.missionId, type: 'plan_ready_for_approval', actor: 'orchestrator', actorType: 'worker', correlationId: job.id, payload: { taskId: task.id, tokens, estimatedCost: true } } }); }
