...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === 'check_status') {
    ...
    try {
      ...
      if (repository.state === 'MERGED' && current.status !== 'completed') { await prisma.$transaction([prisma.task.update({ where: { id }, data: { status: 'completed', completedAt: repository.mergedAt ? new Date(repository.mergedAt) : new Date() } }), prisma.auditEvent.create({ data: { actor: 'task-checker', action: 'task.pull_request_merged', target: id, result: 'success', metadata: { pullRequestUrl: current.pullRequestUrl } } })]); status = 'completed'; }
  ...
  if (body.action === 'request_plan') {
    ...
    await prisma.task.update({ where: { id }, data: { status: 'planning', startedAt: current.startedAt || new Date() } });
    try {
      ...
      await prisma.task.update({ where: { id }, data: { status: current.status } });
  ...
  if (body.action === 'approve_plan' || body.action === 'reject_plan') {
    ...
    if (approved) {
      try {
        ...
        await prisma.$transaction([
          prisma.task.update({ where: { id }, data: { status: 'awaiting_plan_approval', assignedAgent: current.assignedAgent } }),
          prisma.approval.updateMany({ where: { taskId: id, approvalType: 'plan', decision: 'approved' }, data: { decision: 'pending', approvedBy: null, approvedAt: null } }),
          prisma.auditEvent.create({ data: { actor: auth.session!.userId, action: 'task.execution_queued', target: id, result: 'failed' } }),
        ]);
  ...
  if (body.action === 'approve_final' || body.action === 'reject_final') {
    ...
    await prisma.$transaction([
      prisma.task.update({ where: { id }, data: { status: approved ? 'approved_for_merge' : 'rejected' } }),
      prisma.approval.updateMany({ where: { taskId: id, approvalType: 'merge', decision: 'pending' }, data: { decision: approved ? 'approved' : 'rejected', approvedBy: auth.session!.userId, approvedAt: new Date(), comments: typeof body.comments === 'string' ? body.comments.slice(0, 2000) : null } }),
      prisma.auditEvent.create({ data: { actor: auth.session!.userId, action: approved ? 'task.final_approved' : 'task.final_rejected', target: id, result: 'success', metadata: { automaticMerge: false } } }),
    ]);
