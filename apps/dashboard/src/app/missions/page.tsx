...
export default async function MissionsPage() {
  ...
  return <OpsPage eyebrow="OPERATIONS" title="Missions" description="Mission intent, execution state, ownership, economics, and next actions."><section className="panel"><div className="list">{missions.map(m => {
    ...
    return <EntityLink key={m.id} href={`/missions/${m.id}`} title={m.goal} detail={`${m.project?.name ?? 'No workspace'} · ${m.status} · ${complete}/${m.tasks.length} tasks · $${spend.toFixed(2)} / $${m.budgetUsd.toFixed(2)} · ${tokens.toLocaleString()} tokens`} />;
