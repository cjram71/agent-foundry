...
import StartDiscovery from '@/components/start-discovery';
...
export default async function ExecutiveHome() {
  ...
  const setupStage = constitution?.status !== 'ACTIVE' ? 'CONSTITUTION' : company._count.missions === 0 ? 'DISCOVERY' : 'OPERATING';
  ...
  return <div className="page-stack executive-home">
    ...
    <section className="company-journey panel">
      <div className="journey-head"><div><p className="eyebrow">YOUR NEXT STEP</p><h2>{setupStage === 'CONSTITUTION' ? 'Approve how AI may operate' : setupStage === 'DISCOVERY' ? 'Start company discovery' : 'Operate and improve Boosta'}</h2><p>{setupStage === 'CONSTITUTION' ? 'Boosta Förlag is already configured and locked. Review the operating rules; approval establishes boundaries but does not start autonomy.' : setupStage === 'DISCOVERY' ? 'Run bounded public research across Boosta products, markets, finances, security and compliance. Independent Red Team findings are synthesized by the AI CEO for your decision.' : 'Review decisions, monitor approved work and measure business results.'}</p></div><span className="stage-label">{setupStage === 'OPERATING' ? 'Operating' : `Setup ${setupStage === 'CONSTITUTION' ? '1' : '2'} of 2`}</span></div>
      <div className="journey-progress"><span className="complete">✓ <b>Boosta identity locked</b></span><i/><span className={constitution?.status === 'ACTIVE' ? 'complete' : 'current'}>1 <b>Rules approved</b></span><i/><span className={company._count.missions ? 'complete' : setupStage === 'DISCOVERY' ? 'current' : ''}>2 <b>Discovery prepared</b></span></div>
      <div className="journey-action">{setupStage === 'CONSTITUTION' ? <><ConstitutionApproval/><Link className="button secondary" href="/businesses">Read all rules</Link></> : setupStage === 'DISCOVERY' ? <><StartDiscovery/><Link className="button secondary" href="/businesses">Review locked company identity</Link></> : <Link className="button primary" href="/approvals">Review company decisions</Link>}</div>
    </section>
