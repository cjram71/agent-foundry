...
export async function generateRouted(prisma: PrismaClient, taskId: string, role: RoutedRole, prompt: string, options: { jsonSchema?: object; recordRun?: boolean } = {}): Promise<RoutedResult> {
  ...
  const localModel = role === 'coder' ? (process.env.OLLAMA_MODEL || 'qwen2.5-coder:3b') : (process.env.OLLAMA_REVIEWER_MODEL || 'deepseek-coder-v2:16b-lite-instruct-q4_K_M');
  const decision = routeModel({ role: role === 'coder' ? 'coder' : 'reviewer', risk: normalizeRisk(task.riskLevel), privacySensitive: false, complexity: 'complex', estimatedTokens: positive(process.env.RUNNER_ESTIMATED_TOKENS, role === 'coder' ? 32_768 : 8_192), cloudRatePerMillionUsd: rate, cloudAvailable: Boolean(process.env.GEMINI_API_KEY), localAvailable: process.env.OLLAMA_DISABLED !== 'true', cloudModel: process.env.GEMINI_RUNNER_MODEL || 'gemini-3.6-flash', localModel, budget: { tokenLimit: positive(process.env.DEVELOPER_TASK_TOKEN_LIMIT, 180_000), dailyTokenLimit: positive(process.env.DEVELOPER_DAILY_TOKEN_LIMIT, 720_000), maximumTaskCostUsd: positive(process.env.DEVELOPER_MAX_TASK_COST_USD, 8), maximumDailyCostUsd: positive(process.env.DEVELOPER_MAX_DAILY_COST_USD, 40) }, usage: { taskTokens: taskTokens._sum.tokenUsage || 0, dailyTokens: dailyTokens._sum.tokenUsage || 0, taskCloudCostUsd: estimateUsd(taskCloud._sum.tokenUsage || 0, rate), dailyCloudCostUsd: estimateUsd(dailyCloud._sum.tokenUsage || 0, rate) } });
